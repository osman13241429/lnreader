...
export class EdgeTTSClient {
  ...
  private handleMessage(event: any, metadataBuffer: Metadata) {
    ...
    if (message.includes('Path:turn.start')) {
      ...
      const binaryDelimString = EdgeTTSClient.BINARY_DELIM;
      const headerIndex = message.indexOf(binaryDelimString);

      if (headerIndex !== -1) {
        const audioDataStart = headerIndex + binaryDelimString.length;

        const audioData = buffer.subarray(audioDataStart);

        if (audioData.length > 0) {
          this.cacheAudioData(audioData, requestId);
        } else {
          // Received audio message with header but no data
        }
      } else {
        // Delimiter not found in the first chunk, likely subsequent data chunks
      }
  ...
  private cacheAudioData(audioData: Uint8Array, requestId: string) {
  ...
  private buildSSML(text: string, options: ProsodyOptions): string {
    // Escape special characters in the text to make valid XML
...
function generateRandomHex(length: number): string {
  // Use Math.random() as a fallback for environments without window.crypto
  let result = '';
  const characters = '0123456789abcdef';
  const charactersLength = characters.length;
  for (let i = 0; i < length * 2; i++) {
    // length * 2 because each byte needs 2 hex chars
    result += characters.charAt(Math.floor(Math.random() * charactersLength));
  }
  return result.slice(0, length * 2); // Ensure correct length if needed, though loop should handle it
}

/* Original implementation using window.crypto:
function generateRandomHex(length: number): string {
