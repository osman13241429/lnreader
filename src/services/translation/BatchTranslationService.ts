...
export const batchTranslateChapters = async (
  ...
  console.log(
    `🔥 [BATCH API] RECEIVED ${chapters.length} CHAPTERS FOR PROCESSING`,
  );
  ...
  console.log('🔍 [BATCH API] Novel data:', {
    id: novel.id,
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log(
                '🔍 [BATCH API] Checking which chapters need downloading...',
              );
              ...
              const alreadyDownloaded = normalizedChapters.filter(
                (chapter, index) => existsResults[index],
              );

              console.log(
                `🔍 [BATCH API] Need to download ${chaptersToDownload.length} chapters, ${alreadyDownloaded.length} already downloaded`,
              );
              ...
              if (chaptersToDownload.length > 0) {
                ...
                console.log(
                  '🔍 [BATCH API] All chapters already downloaded, proceeding to translation',
                );
                showToast('Processing translations...');
              }
              ...
              for (const chapter of normalizedChapters) {
                ...
                console.log(
                  `🔍 [BATCH API] Chapter ${chapterId} file exists: ${exists}`,
                );
                ...
                if (exists) {
                  ...
                  console.log(
                    `🔍 [BATCH API] Warning: Chapter ${chapterId} still not downloaded, will skip translation`,
                  );
                }
              }

              console.log(
                `🔍 [BATCH API] Proceeding with translations for ${readyForTranslation.length} chapters`,
              );
              ...
              console.error(
                '🔍 [BATCH API] Error in download/translate process:',
                error,
              );
...
const processTranslations = async (
  ...
  console.log(
    `🔍 [BATCH TRANSLATE] Processing ${chapters.length} chapters for translation`,
  );
  ...
  for (const chapter of chapters) {
    try {
      ...
      if (!chapterId) {
        console.error(
          '🔍 [BATCH TRANSLATE] Cannot translate chapter without ID:',
          chapter,
        );
        continue;
      }
      ...
      if (hasTranslation) {
        console.log(
          `🔍 [BATCH TRANSLATE] Chapter ${chapterId} already has a translation, skipping`,
        );
        continue;
      }
      ...
      console.log(
        `🔍 [BATCH TRANSLATE] Checking file for translation: ${filePath}`,
      );
      const fileExists = await FileManager.exists(filePath);
      if (!fileExists) {
        console.log(
          `🔍 [BATCH TRANSLATE] Skipping chapter ${chapterId} as it's not downloaded (file doesn't exist)`,
        );
      ...
      console.log(
        `🔍 [BATCH TRANSLATE] Reading file content for chapter ${chapterId}`,
      );
      const chapterContent = await FileManager.readFile(filePath);

      if (!chapterContent || chapterContent.trim() === '') {
        console.warn(
          `🔍 [BATCH TRANSLATE] Chapter ${chapterId} content is empty, skipping`,
        );
        continue;
      }
      ...
      console.log(
        `🔍 [BATCH TRANSLATE] Translating chapter ${chapterId} (content length: ${chapterContent.length})`,
      );
      ...
      try {
        ...
        console.log(
          `🔍 [BATCH TRANSLATE] Processing translated content for chapter ${chapterId}`,
        );
        ...
        console.log(
          `🔍 [BATCH TRANSLATE] Saving translation for chapter ${chapterId} to database`,
        );
        ...
        console.log(
          `🔍 [BATCH TRANSLATE] Updating hasTranslation flag for chapter ${chapterId}`,
        );
        db.transaction(tx => {
          tx.executeSql(
            ...
            () => {
              console.log(
                `🔍 [BATCH TRANSLATE] Successfully updated hasTranslation flag for chapter ${chapterId}`,
              );
            },
            (_: any, error: any) => {
              console.error(
                '🔍 [BATCH TRANSLATE] Error updating hasTranslation flag:',
                error,
              );
              return false;
            },
        ...
        console.log(
          `🔍 [BATCH TRANSLATE] Successfully translated chapter ${chapterId}`,
        );
        successCount++;
      } catch (translationError) {
        // Handle translation-specific errors
        console.error(
          `🔍 [BATCH TRANSLATE] Translation API error for chapter ${chapterId}:`,
          translationError,
        );
      ...
      console.error(
        '🔍 [BATCH TRANSLATE] Error processing chapter:',
        error,
        chapter,
      );
    }
  ...
  console.log(
    `🔍 [BATCH TRANSLATE] Completed translation of ${successCount}/${chapters.length} chapters`,
  );
...
const attemptDownloadChapters = async (
  ...
  for (const chapter of chaptersToDownload) {
    try {
      ...
      if (!chapterId) {
        console.error(
          '🔍 [BATCH API] Cannot download chapter without ID:',
          chapter,
        );
        continue;
      }
      ...
      console.log(
        `🔍 [BATCH API] Queuing download for chapter ${chapterId} (attempt ${
          retryAttempt + 1
        })`,
      );
      ...
      console.error(
        '🔍 [BATCH API] Error queueing chapter download:',
        error,
        chapter,
      );
    }
  ...
  console.log(
    `🔍 [BATCH API] Queued ${
      chapterIdsBeingDownloaded.length
    } chapters for download (attempt ${retryAttempt + 1})`,
  );
  ...
  try {
    ...
    while (!queueEmpty && Date.now() - startTime < MAX_WAIT_TIME) {
      ...
      if (!pendingDownloads) {
        console.log(
          '🔍 [BATCH API] All downloads have been processed by the queue',
        );
        queueEmpty = true;
        break;
      }
      ...
      console.log(
        `🔍 [BATCH API] Found ${pendingDownloads} download tasks in queue, waiting...`,
      );
    ...
    if (!queueEmpty) {
      console.log(
        '🔍 [BATCH API] Maximum wait time exceeded, proceeding anyway',
      );
    }

    // Additional verification: check if files actually exist after download queue is clear
    console.log(
      '🔍 [BATCH API] Verifying downloads succeeded by checking files...',
    );
    ...
    for (const chapter of chaptersToDownload) {
      ...
      const exists = await FileManager.exists(filePath);
      console.log(
        `🔍 [BATCH API] Chapter ${chapterId} download status: ${
    ...
    if (failedDownloads.length > 0 && retryAttempt < MAX_RETRIES) {
      console.log(
        `🔍 [BATCH API] ${failedDownloads.length} downloads failed, retrying...`,
      );
      ...
      console.log(
        `🔍 [BATCH API] ${failedDownloads.length} downloads failed after all retries`,
      );
