...
import { saveTranslation, getTranslation, deleteTranslation } from '@database/queries/TranslationQueries';
...
export const useTranslation = (chapterId: number) => {
  ...
  const translateChapter = useCallback(async (chapter: ChapterInfo, novel: { pluginId: string; id: number; name?: string }) => {
    if (!apiKey) {
    ...
    if (!chapter || !novel || !novel.pluginId || !novel.id || !chapter.id) {
      showToast(safeGetString('common.error') + ': Invalid chapter or novel data');
      return;
    }
    ...
    try {
      ...
      db.transaction(tx => {
        tx.executeSql(
          ...
          [chapter.id],
          () => {
            console.log(`Successfully updated hasTranslation flag for chapter ${chapter.id}`);
          },
          (_, error) => {
  ...
  const removeTranslation = useCallback(async () => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      showToast(safeGetString('translation.deleteError', { error: errorMessage }));
    }
  ...
  const exportTranslation = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      showToast(safeGetString('translation.exportError', { error: errorMessage }));
      return null;
    }
