...
export const useTranslateBatch = () => {
  ...
  const waitForFileDownload = async (filePath: string, maxAttempts = 30, intervalMs = 1000) => {
  ...
  const waitForDownloadsToComplete = async (
    ...
    while (pending.length > 0) {
      ...
      if (Date.now() - startTime > maxWaitTimeMs) {
        console.log(`Exceeded maximum wait time (${maxWaitTimeMs}ms) for downloads`);
        return false;
      }
  ...
  const translateChapters = useCallback(async (items: any[]) => {
    if (!apiKey) {
    ...
    for (const novelKey in novelChapters) {
      ...
      const downloadFilePaths = chaptersToDownload.map(chapter => 
      ...
      if (chaptersToDownload.length > 0) {
          ...
          try {
            ServiceManager.manager.addTask({
              ...
              data: {
                ...
                chapterName: chapter.name || chapter.chapterTitle || `Chapter ${chapter.id}`
              }
            });
          } catch (error) {
        ...
        showToast(`Waiting for ${chaptersToDownload.length} downloads to complete...`);
        ...
        if (!allDownloaded) {
          showToast(`Some downloads did not complete. Proceeding with available chapters.`);
        }
      }
      ...
      for (const chapter of chapters) {
        try {
          ...
          if (!await FileManager.exists(filePath)) {
            console.warn(`Chapter ${chapter.id} not available for translation, skipping`);
            continue;
          }
